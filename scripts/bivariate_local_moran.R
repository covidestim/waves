...
for (i in days) {
  ...
  lisa_bivariate <- local_bimoran(queen_w,
                                  filtered[,c("trend_delta", "immune_alpha")])
...
fig5b <- joined_df |> 
  ...
  geom_sf(data = us_states,
          color = "deeppink4",
          fill = "transparent")+
  # scale_fill_manual(name = "Moran's I clustering",
  #                   values = c("#0000FF","#a7adf9","#eeeeee","#f4ada8","#FF0000"))+
  # scale_color_manual(name = "Moran's I clustering",
  #                    values = c("#0000FF","#a7adf9","#eeeeee","#f4ada8","#FF0000"))+
  theme_void(base_size = 12)+
  theme(legend.position = "bottom",
        legend.title = element_text(hjust = 0.5),
