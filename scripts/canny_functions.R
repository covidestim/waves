...
canny_edge_detector <- function(m, 
                                gaussian_kernel = c("3x3","5x5"),
                                low_threshold, 
                                high_threshold){
  require(OpenImageR)
  
  if(!is.matrix(m)){
    m <- as.matrix(Matrix::sparseMatrix(i = m$i,
                                             j = m$j,
                                             x = m$alpha))
  }
  
  #Step.1 Denoiesing with Gaussian kernel
  if(missing(gaussian_kernel)){
    message("Using 3x3 Gaussian Kernel")
    gaussian_kernel <- "3x3"
  }
  
  if(gaussian_kernel == "3x3"){
    gaussian_kernel <- matrix(nrow = 3,
                              c(1,2,1,2,4,2,1,2,1)/16)
    
  }else if(gaussian_kernel == "5x5"){
    gaussian_kernel <- matrix(nrow = 5,
                              c(2,4,5,4,2,
                                4,9,12,9,4,
                                5,12,15,12,5,
                                4,9,12,9,4,
    ...
    stop("Gaussian Kernel should be one of the options, '3x3' or '5x5'")
  }
