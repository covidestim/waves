...
# boundList <- readRDS("boundaryPlots/firstNewModel/boundaryDatafirst.rds")
...
for (i in 2:length(boundList)){
  ...
  recruit[i-1] <- (nrow(boundList[[i]][["wave"]]) - nrow(boundList[[i-1]][["wave"]]))*64.75
...
### Create a dataframe of boundary length and growth at time t
summary1DF <- data.frame("Days before peak" = 62:0, 
                         "Wave edge length (km)" = as.vector(unlist(sapply(boundList, function(x) x['boundary length', drop = FALSE])))[-1]/1000, 
                         "Recruitment rate (km2/day)" = recruit, 
                         "Median speed (km/day)" = sapply(distanceToFrontier, function(x) median(as.numeric(unlist(x[,'distToFront', drop = FALSE])), na.rm = TRUE))/1000,
                         check.names = FALSE
)

write.csv(summary1DF, file = here("data-products/wave1Characteristics.csv"))
...
# distanceToFrontier <- readRDS(here("boundaryPlots/firstNewModel/firstDistanceToFrontier.rds"))
...
saveRDS(boundList, file = "boundaryPlots/secondNewModel/boundaryData_secondWave.rds", version = 2)

# boundList <- readRDS(here("boundaryPlots/secondNewModel/boundaryData_secondWave.rds"))
...
### Create a dataframe of boundary length and growth at time t
summary2DF <- data.frame("Days before peak" = 62:0, 
                      "Wave edge length (km)" = as.vector(unlist(sapply(boundList, function(x) x['boundary length', drop = FALSE])))[-1]/1000, 
                      "Recruitment rate (km2/day)" = recruit, 
                      "Median speed (km/day)" = sapply(distanceToFrontier, function(x) median(as.numeric(unlist(x[,'distToFront', drop = FALSE])), na.rm = TRUE))/1000,
                      check.names = FALSE
...
write.csv(summary2DF, file = here("data-products/wave2Characteristics.csv"))

### Save for future use ### 
saveRDS(distanceToFrontier, here("boundaryPlots/secondNewModel/secondDistanceToFrontier.rds"), version = 2)
