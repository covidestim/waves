...
hyper_smooth_bym2 <- list(
  phi = list(prior = "pc", param = c(0.95, 0.8),
             initial = 0.9),
...
days <- test_dates[5:12]
# days <- as.Date("2020-09-26")
###############################################################################
##### Run the model 
##############################################################################|
...
ggplot_object <- last_plot()

ggplot2::ggsave(filename = "../CHECKING WAVES/forRafaTest.png", plot = ggplot_object, width = 9, height = 12, dpi = 300)
