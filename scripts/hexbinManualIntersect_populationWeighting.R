...
for (i in 1:length(unique(na.omit(observationFips$date)))){
  ...
  ggplot() + geom_sf(output_shapes, mapping = aes(fill = immune))+
    theme_minimal() +
    scale_fill_gradient(low = "thistle1", high = "deeppink4")  +
    geom_sf(output_shapes %>% filter(immune == 0), mapping = aes(), fill="green")
...
# %>% 
#   mutate(infectionsPC = ifelse(population == 0, 0, (infections/population)*1e5))
...
if(sum(observationFips %>% 
   ...
   sum(hexObservationsAll %>% 
       filter(date == testDate, is.na(immune)==FALSE) %>% 
       select(immune)) > 1) {
...
### Check with a plot 
ggplot() +
  geom_sf(hexObservationsAllSF %>% filter(date == testDate, immune>0),
          mapping=aes(fill=immune)) +
  scale_fill_gradient(low = "thistle1", high = "deeppink4") +
  geom_sf(hexObservationsAllSF %>% filter(immune == 0),
          mapping=aes(), fill="green") +
  geom_sf(observationFips %>% filter(date == testDate, immune == 0), 
...
hexObservationsAll$immune <- as.numeric(hexObservationsAll$immune)
hexObservationsAll$immune <- as.numeric(hexObservationsAll$immune)
### Remove missing values for infectionsPC 
hexObservationsAllNoMissing <- hexObservationsAll %>% filter(is.na(immune) == FALSE)
...
testDate <- sort(unique(na.omit(observationFips$date)))[708]
...
st_write(obj = hexObservationsAllNoMissingGeom,
         paste0("data-products/geo-hexes/hexid-observations_", args$modelVersion, ".geojson"),
         delete_dsn = T)
