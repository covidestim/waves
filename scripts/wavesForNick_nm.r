...
# days <- (seq.Date(from = delta_peak-63, to = delta_peak, by = "day"))
  ...
  ####### Plot
  ## Color levels
  mx = max(alphaPreds200,alphaPreds500,na.rm=T)
  cuts <- seq(0,mx,length.out=51)
  alphaPreds200_lvls <- floor(alphaPreds200/(mx+1e-9)*50)+1
  alphaPreds500_lvls <- floor(alphaPreds500/(mx+1e-9)*50)+1

    # k=200
  library(RColorBrewer)
  cls <-  colorRampPalette(brewer.pal(11, "RdYlBu"))(50)[50:1]
 
  pdfnam <- "Fig_alpha_wave_k200_Nov-16-2025.pdf"
  ...
  points(coords2$X,coords2$Y,col=cls[alphaPreds200_lvls[,dy]],pch=16,cex=0.7)
  ...
  # k=500
  pdfnam <- "Fig_alpha_wave_k500_Nov-16-2025.pdf"
  pdf(file=pdfnam,width=9, height=6.5) 
  par(mfrow=c(7,5),mar=c(0,0,0,0),oma=c(1,1,1,1))
  
  for(dy in 1:64){
    plot(NA,NA,xlim=range(coords2$X),ylim=range(coords2$Y),axes=F,xlab=NA,ylab=NA)
    points(coords2$X,coords2$Y,col=cls[alphaPreds500_lvls[,dy]],pch=16,cex=0.7)
    text(-2.6*1e6,350000,alpha_dates[dy],pos=4,cex=0.9)
    box(lwd=0.3)
  }
  ...
  # save
  save(alphaPreds200,alphaPreds500,coords2,file="alpha_wave_fits_Nov-16-2025.rData")
  ...
  ####### Plot
  ## Color levels
  mx = max(deltaPreds200,deltaPreds500,na.rm=T)
  cuts <- seq(0,mx,length.out=51)
  deltaPreds200_lvls <- floor(deltaPreds200/(mx+1e-9)*50)+1
  deltaPreds500_lvls <- floor(deltaPreds500/(mx+1e-9)*50)+1
  
  # k=200
  library(RColorBrewer)
  cls <-  colorRampPalette(brewer.pal(11, "RdYlBu"))(50)[50:1]
  
  pdfnam <- "Fig_delta_wave_k200_Nov-16-2025.pdf"
  pdf(file=pdfnam,width=9, height=6.5) 
  par(mfrow=c(7,5),mar=c(0,0,0,0),oma=c(1,1,1,1))
  
  for(dy in 1:64){
    plot(NA,NA,xlim=range(coords2$X),ylim=range(coords2$Y),axes=F,xlab=NA,ylab=NA)
    points(coords2$X,coords2$Y,col=cls[deltaPreds200_lvls[,dy]],pch=16,cex=0.7)
  ...
  # k=500
  pdfnam <- "Fig_delta_wave_k500_Nov-16-2025.pdf"
  pdf(file=pdfnam,width=9, height=6.5) 
  par(mfrow=c(7,5),mar=c(0,0,0,0),oma=c(1,1,1,1))
  
  for(dy in 1:64){
    plot(NA,NA,xlim=range(coords2$X),ylim=range(coords2$Y),axes=F,xlab=NA,ylab=NA)
    points(coords2$X,coords2$Y,col=cls[deltaPreds500_lvls[,dy]],pch=16,cex=0.7)
    text(-2.6*1e6,350000,delta_dates[dy],pos=4,cex=0.9)
    box(lwd=0.3)
  }
  ...
  # save
  save(deltaPreds200,deltaPreds500,coords2,file="delta_wave_fits_Nov-16-2025.rData")
